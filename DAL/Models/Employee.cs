using System;
using System.Collections.Generic;

namespace DAL.Models;

public partial class Employee
{
    public int EmployeeId { get; set; }

    public string? EmployeCode { get; set; }

    public string? Title { get; set; }

    public string? EmployeeName { get; set; }

    public string? FatherName { get; set; }

    public string? SpouseName { get; set; }

    public DateTime? Dob { get; set; }

    public string? Gender { get; set; }

    public string? PhoneNo { get; set; }

    public string? MobileNo { get; set; }

    public string? EmailId { get; set; }

    public string? PanNo { get; set; }

    public int? DepartmentId { get; set; }

    public int? DesignationId { get; set; }

    public int? SalaryGradeId { get; set; }

    public int? EmployeeStatusId { get; set; }

    public int? MaritalStatusId { get; set; }

    public string? BloodGroup { get; set; }

    public string? CardNumber { get; set; }

    public DateTime? JoiningDate { get; set; }

    public int? ProbationPeriod { get; set; }

    public DateTime? ConfirmationDate { get; set; }

    public string? ImageUrl { get; set; }

    public int? BankId { get; set; }

    public int? BankBranchId { get; set; }

    public string? Ifsccode { get; set; }

    public string? AccountNo { get; set; }

    public string? PaymentType { get; set; }

    public string? DdpayableAt { get; set; }

    public string? NameAsPerBank { get; set; }

    public string? AadharEnrollmentNo { get; set; }

    public string? NameAsPerAadhar { get; set; }

    public string? AadharNo { get; set; }

    public bool? IncludedInPfscheme { get; set; }

    public DateTime? PfjoinDate { get; set; }

    public string? Pfscheme { get; set; }

    public string? Pfnumber { get; set; }

    public bool? EpfexcessContribution { get; set; }

    public bool? EpsexcessContribution { get; set; }

    public bool? IncludedInEsischeme { get; set; }

    public string? Esino { get; set; }

    public DateTime? ResignSubmitted { get; set; }

    public bool? NoticeRequired { get; set; }

    public bool? NoticeServed { get; set; }

    public int? NoticePeriod { get; set; }

    public bool? IsDeceased { get; set; }

    public DateTime? DemiseDate { get; set; }

    public DateTime? RetireDate { get; set; }

    public bool? HasLeft { get; set; }

    public DateTime? LeavingDate { get; set; }

    public int? LeavingReasonId { get; set; }

    public string? EmployeeFeedback { get; set; }

    public DateTime? FinalSettelementdate { get; set; }

    public DateTime? RelieveDate { get; set; }

    public DateTime? ExitInterviewDate { get; set; }

    public bool? FitRehired { get; set; }

    public string? Remarks { get; set; }

    public string? IqamaNo { get; set; }

    public string? PassportNo { get; set; }

    public DateTime? ValidUpto { get; set; }

    public string? VisaNo { get; set; }

    public int? VisaType { get; set; }

    public int? LocationId { get; set; }

    public int? CompanyId { get; set; }

    public bool? EmployeeActive { get; set; }

    public string? EntryType { get; set; }

    public string? RelationShip { get; set; }

    public decimal? BasicSalary { get; set; }

    public int? OfferLetterId { get; set; }

    public bool? ConfirmationStatus { get; set; }

    public string? CofirLetterDate { get; set; }

    public int? NoOfTimePrintLetter { get; set; }

    public DateTime? AnniversaryDate { get; set; }

    public string? OtherBranch { get; set; }

    public string? NomineeName { get; set; }

    public decimal? Pfuannumber { get; set; }

    public bool? VerifyStatus { get; set; }

    public int? IssuedAppointmentLatter { get; set; }

    public int? ZoneId { get; set; }

    public bool? TdsincludeInSalary { get; set; }

    public int? ChangedNoticeDays { get; set; }

    public bool? PtincludeInSalary { get; set; }
}
